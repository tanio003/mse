function [PopulationSolution] = parseBGSSolutions(FullSolution)
%% parseBGSSolutions
% Generate structures from FullSolution for each sample bulk metagenomes
% Potentially expand this to each ecotype (fittest strain), and the abundance weighted population
...
for a = 1:Gridding.nStr
    PopulationSolution.Growth(:,a) = FullSolution.(Gridding.strNameVec{a}).Growth;
    PopulationSolution.Fluxes(:,:,a) = FullSolution.(Gridding.strNameVec{a}).Fluxes;
    PopulationSolution.Shadow(:,:,a) = FullSolution.(Gridding.strNameVec{a}).Shadow;
    PopulationSolution.BOF_coefs(:,:,a) = FullSolution.(Gridding.strNameVec{a}).BOF_coefs;
    PopulationSolution.TpOpt(:,:,a) = FullSolution.(Gridding.strNameVec{a}).TpOpt;
    PopulationSolution.r_opt(:,a) = FullSolution.(Gridding.strNameVec{a}).r_opt;
    PopulationSolution.pigAbs(:,:,a) = FullSolution.(Gridding.strNameVec{a}).pigAbs;
    PopulationSolution.S_star(:,:,a) = FullSolution.(Gridding.strNameVec{a}).S_star;
    PopulationSolution.uptakeBounds(:,:,a) = FullSolution.(Gridding.strNameVec{a}).uptakeBounds;
    PopulationSolution.StrMod1_growth(:,a) = FullSolution.(Gridding.strNameVec{a}).StrMod1_growth;
    PopulationSolution.StrMod2_growth(:,a) = FullSolution.(Gridding.strNameVec{a}).StrMod2_growth;
    PopulationSolution.StrMod3_growth(:,a) = FullSolution.(Gridding.strNameVec{a}).StrMod3_growth;
    PopulationSolution.StrMod4_growth(:,a) = FullSolution.(Gridding.strNameVec{a}).StrMod4_growth;
    PopulationSolution.StrMod5_growth(:,a) = FullSolution.(Gridding.strNameVec{a}).StrMod5_growth;
end

%% Commented J.Casey's code for parsing out for each strain and ecotypes for now.
