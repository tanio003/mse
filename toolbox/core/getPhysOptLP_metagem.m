function [physOptLP] = getPhysOptLP_metagem(StrMod, FileNames, Constraints, a, PigsIncluded, maxIter)
...
% Get ecotype index
% For HLIII_HLIV assume they are same as HLII as there are no experiemental
% data
sampleName = StrMod.id;
CruiseDatabase = readtable(FileNames.CruiseDB_filename,'ReadVariableNames',true,'Delimiter',',');
SampleInd = find(contains(CruiseDatabase.Station,sampleName));
